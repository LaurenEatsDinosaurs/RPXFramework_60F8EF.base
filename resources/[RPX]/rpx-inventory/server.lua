...
function server.setPlayerInventory(player, data)
	...
	if inv then
		...
		TriggerClientEvent('rpx-inventory:setPlayerInventory', player.source, Inventory.Drops, inventory, totalWeight, inv.player)
	end
end
exports('setPlayerInventory', server.setPlayerInventory)
AddEventHandler('rpx-inventory:setPlayerInventory', server.setPlayerInventory)

lib.callback.register('rpx-inventory:openInventory', function(source, inv, data)
	...
	if right then
		...
		if right.player then
			TriggerClientEvent('rpx-inventory:closeInventory', right.player.source, true)
		end
...
lib.callback.register('rpx-inventory:buyLicense', function(source, id)
...
lib.callback.register('rpx-inventory:getItemCount', function(source, item, metadata, target)
	local inventory = target and Inventory(target) or Inventory(source)
	return (inventory and Inventory.GetItem(inventory, item, metadata, true)) or 0
end)

lib.callback.register('rpx-inventory:getInventory', function(source, id)
...
lib.callback.register('rpx-inventory:useItem', function(source, itemName, slot, metadata)
	local inventory = Inventory(source)
	...
	if inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			local success = lib.callback.await('rpx-inventory:usingItem', source, data)
					...
					if data.count > 1 then
						...
						if emptySlot then
							...
							if newItem then
								...
								TriggerClientEvent('rpx-inventory:updateSlots', inventory.id, {
									{
										item = newItem,
...
lib.addCommand('clearevidence', {
	...
	if hasPermission then
		MySQL.query('DELETE FROM rpx-inventory WHERE name = ?', {('evidence-%s'):format(args.evidence)})
	end
end)
...
lib.addCommand('viewinv', {
	...
	if inventory then TriggerClientEvent('rpx-inventory:viewInventory', source, inventory) end
end)

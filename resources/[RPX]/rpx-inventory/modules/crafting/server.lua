...
lib.callback.register('rpx-inventory:openCraftingBench', function(source, id, index)
	...
	if bench then
		...
		if left.open and left.open ~= source then
			...
			if inv then
				if inv.player then
					TriggerClientEvent('rpx-inventory:closeInventory', inv.owner, true)
				end
...
lib.callback.register('rpx-inventory:craftItem', function(source, id, index, recipeId, toSlot)
	local left, bench = Inventory(source), CraftingBenches[id]
	...
	if bench then
		...
		if recipe then
			...
			local success = lib.callback.await('rpx-inventory:startCrafting', source, id, recipeId)
			...
			if success then
				...
				for slot, count in pairs(tbl) do
					...
					if count < 1 then
						...
						if invSlot.count > 1 then
							...
							if emptySlot then
								...
								if newItem then
									...
									TriggerClientEvent('rpx-inventory:updateSlots', left.id, {
										{
											item = newItem,
											inventory = left.type
										}
						...
						TriggerClientEvent('rpx-inventory:updateSlots', source, {
							{
								item = invSlot,
								inventory = left.type
							}

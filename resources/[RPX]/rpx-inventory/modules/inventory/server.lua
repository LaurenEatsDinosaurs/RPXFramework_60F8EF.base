...
local function loadInventoryData(data, player)
	...
	if not data.type and type(data.id) == 'string' then
		if data.id:find('^evidence-') then
			data.type = 'policeevidence'
		end
	end

	if data.type == 'policeevidence' then
...
CreateThread(function()
	TriggerEvent('rpx-inventory:loadInventory', Inventory)
			...
			if inv.player then
				...
				if not ignore[hash] then
					...
					if not inv.weapon then
						TriggerClientEvent('rpx-inventory:disarm', id)
					end
				end
			end
		end
...
--- To create a stash, please use `exports['rpx-inventory']:RegisterStash` instead.
...
function Inventory.Remove(inv)
	...
	if inv then
		if inv.type == 'drop' then
			TriggerClientEvent('rpx-inventory:removeDrop', -1, inv.id)
...
function Inventory.SetDurability(inv, slot, durability)
	...
	if inv and slotData then
		...
		if inv.player then
			...
			TriggerClientEvent('rpx-inventory:updateSlots', inv.id, {{item = slotData, inventory = inv.type}}, {left=inv.weight, right=inv.open and Inventories[inv.open]?.weight})
		end
	end
end
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	if type(toSlot) == 'number' then
		...
		if inv.player then
			if server.syncInventory then server.syncInventory(inv) end
			TriggerClientEvent('rpx-inventory:updateSlots', inv.id, {{ item = inv.items[toSlot], inventory = inv.type }}, { left = inv.weight, right = inv.open and Inventories[inv.open]?.weight }, slotCount, false)
		end
		...
		if inv.player then
			if server.syncInventory then server.syncInventory(inv) end
			TriggerClientEvent('rpx-inventory:updateSlots', inv.id, toSlot, { left = inv.weight, right = inv.open and Inventories[inv.open]?.weight }, added, false)
		end
	...
	if count > 0 then
		...
		if removed > 0 then
			...
			if inv.player then
				...
				TriggerClientEvent('rpx-inventory:updateSlots', inv.id, array, {left=inv.weight, right=inv.open and Inventories[inv.open]?.weight}, removed, true)
...
---@param metadata table
---@param slot number
RegisterServerEvent('rpx-inventory:removeItem', function(name, count, metadata, slot)
...
local function CustomDrop(prefix, items, coords, slots, maxWeight, instance, model)
	...
	TriggerClientEvent('rpx-inventory:createDrop', -1, dropId, Inventory.Drops[dropId])
end

AddEventHandler('rpx-inventory:customDrop', CustomDrop)
...
local function dropItem(source, data)
	...
	TriggerClientEvent('rpx-inventory:createDrop', -1, dropId, Inventory.Drops[dropId], playerInventory.open and source, slot)
...
lib.callback.register('rpx-inventory:swapItems', function(source, data)
			...
			if toInventory and fromInventory and (fromInventory.id ~= toInventory.id or data.fromSlot ~= data.toSlot) then
				...
				if fromData and (not fromData.metadata.container or fromData.metadata.container and toInventory.type ~= 'container') then
						...
						if not sameInventory then
							if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
								...
								if fromOtherPlayer then
									TriggerClientEvent('rpx-inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', fromData.count })
									TriggerClientEvent('rpx-inventory:itemNotify', fromInventory.id, { toData, 'ui_added', toData.count })
								elseif toOtherPlayer then
									TriggerClientEvent('rpx-inventory:itemNotify', toInventory.id, { fromData, 'ui_added', fromData.count })
									TriggerClientEvent('rpx-inventory:itemNotify', toInventory.id, { toData, 'ui_removed', toData.count })
								end
					...
					elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
						...
						if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
							...
							if not sameInventory then
								...
								if fromOtherPlayer then
									TriggerClientEvent('rpx-inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', data.count })
								elseif toOtherPlayer then
									TriggerClientEvent('rpx-inventory:itemNotify', toInventory.id, { toData, 'ui_added', data.count })
								end
						...
						if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
							...
							if not sameInventory then
								...
								if fromOtherPlayer then
									TriggerClientEvent('rpx-inventory:itemNotify', fromInventory.id, { fromData, 'ui_removed', data.count })
								elseif toOtherPlayer then
									TriggerClientEvent('rpx-inventory:itemNotify', toInventory.id, { fromData, 'ui_added', data.count })
								end
					...
					if sameInventory and fromOtherPlayer then
						TriggerClientEvent('rpx-inventory:updateSlots', fromInventory.id,{
							{
					...
					elseif toOtherPlayer then
						TriggerClientEvent('rpx-inventory:updateSlots', toInventory.id,{
							{
					...
					if toInventory.weapon == data.toSlot then
						if not sameInventory then
							...
							TriggerClientEvent('rpx-inventory:disarm', toInventory.id)
						else
					...
					if fromInventory.weapon == data.fromSlot then
						if not sameInventory then
							...
							TriggerClientEvent('rpx-inventory:disarm', fromInventory.id)
...
function Inventory.Confiscate(source)
	...
	if inv?.player then
		...
		TriggerClientEvent('rpx-inventory:inventoryConfiscated', inv.id)
...
function Inventory.Return(source)
	...
	if inv?.player then
		MySQL.scalar('SELECT data FROM rpx-inventory WHERE name = ?', { inv.owner }, function(data)
			if data then
				MySQL.query('DELETE FROM rpx-inventory WHERE name = ?', { inv.owner })
				...
				TriggerClientEvent('rpx-inventory:inventoryReturned', source, {inventory, totalWeight})
...
function Inventory.Clear(inv, keep)
	...
	if not inv.player then
		if inv.open then
			...
			TriggerClientEvent('rpx-inventory:closeInventory', playerInv.id, true)
	...
	TriggerClientEvent('rpx-inventory:updateSlots', inv.id, updateSlots, { left = inv.weight, right = inv.open and Inventories[inv.open]?.weight })
end
...
SetInterval(function()
	...
	for _, inv in pairs(Inventories) do
		if not inv.open then
			...
			if not inv.player and (inv.datastore or inv.owner) and time - inv.time >= 1200 then
	...
	db.saveInventories(parameters[1], parameters[2])
...
function Inventory.SaveInventories(lock)
	...
	TriggerClientEvent('rpx-inventory:closeInventory', -1, true)
...
RegisterServerEvent('rpx-inventory:closeInventory', function()
...
RegisterServerEvent('rpx-inventory:giveItem', function(slot, target, count)
...
RegisterServerEvent('rpx-inventory:updateWeapon', function(action, value, slot, specialAmmo)
	local inventory = Inventories[source]
	...
	if type == 'table' and action == 'component' then
			...
			if item.metadata.components then
				for k, v in pairs(item.metadata.components) do
					if v == value.component then
						...
						return TriggerClientEvent('rpx-inventory:updateSlots', inventory.id, {{item = item}}, {left=inventory.weight})
					end
				end
			end
		end
		...
		if weapon and weapon.metadata then
			...
			if action ~= 'throw' then TriggerClientEvent('rpx-inventory:updateSlots', inventory.id, {{ item = weapon }}, { left = inventory.weight }) end
...
lib.callback.register('rpx-inventory:removeAmmoFromWeapon', function(source, slot)
	...
	local amtTogive = math.ceil(slotData.metadata.ammo/25)
	if amtTogive < 1 then return end

	if Inventory.AddItem(inventory, item.ammoname, amtTogive, { type = slotData.metadata.specialAmmo or nil }) then
		slotData.metadata.ammo = 0
		slotData.weight = Inventory.SlotWeight(item, slotData)

		TriggerClientEvent('rpx-inventory:updateSlots', inventory.id, {{ item = slotData }}, { left = inventory.weight })
